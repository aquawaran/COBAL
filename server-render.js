...
// Настройка Multer для загрузки музыки
...
// Загрузка музыки
app.post('/api/music/upload', authenticateToken, musicUpload.array('music', 10), async (req, res) => {
    try {
        ...
        for (const file of req.files) {
            ...
            const metadata = await extractMusicMetadata(file.path);
        ...
        res.status(500).json({ error: 'Ошибка загрузки музыки' });
    }
});
...
app.get('/api/music/list', authenticateToken, async (req, res) => {
    try {
        ...
        // Пока возвращаем список файлов из директории
        ...
        console.error('Ошибка получения списка музыки:', error);
        res.status(500).json({ error: 'Ошибка получения списка музыки' });
    }
});
...
app.delete('/api/music/:trackId', authenticateToken, async (req, res) => {
    try {
        ...
        // В реальном приложении здесь будет проверка прав и удаление из БД
        // Пока просто удаляем файл
        const musicDir = path.join(__dirname, 'uploads', 'music');
...
async function extractMusicMetadata(filePath) {
    try {
        // Здесь можно использовать библиотеку вроде music-metadata
...
// Обслуживаем статические файлы (включая музыку)
